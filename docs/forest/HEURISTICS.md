...
- **Epoxidation / EpoxideHydration `dearomatizes` capability.** Catalog `Effect.dearomatizes=true` (aromatic epoxidation / diol look-ahead clears the ring bit at the site). [`PatternInfo::resolve_for_match`](../../crates/xenosite-forest/src/pattern.rs) resolves false on aliphatic site_map atoms (context mol, not Kekulé form). Site-shell residual reads the resolved bit for `|Δaromatic|`. Same resolve shape as Hydrogenation / pair `merge_dearomatizes`. Status: approved (Rust derisk). Tests: `epoxidation_dearomatizes_capability_resolves_on_aromatic_site` / `epoxidation_aromatic_residual_drops_with_dearomatic` / `epoxide_hydration_pattern_info_and_plan` / always-on [`pattern_info_catalog`](../../crates/xenosite-forest/tests/pattern_info_catalog.rs).
