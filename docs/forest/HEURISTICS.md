...
- **Rust door candidates (site–pattern–parent).** `RuleSet::candidates` yields deferred triples; `Candidate::materialize` runs the edit. `find_path` filters by reading `PatternInfo` / `Effect` before materialize (`mol_edits` only counts survivors). Filter closures on `metabolize` stay for Python parity. ResonancePair path emissions are still joint (`pair_emissions`), not yet deferred triples. Status: approved for the derisk door.
