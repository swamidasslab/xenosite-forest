...
- **Use yielded plans mid-search (deprioritize / prune).** After a hit is yielded, expand **soft-demotes** hops whose leaf rule + site (unique-edit orbit) already appears in a yielded plan: (1) expand `order_key` sorts those sites later; (2) heap key is `(target_hit, novel_site, seq)` so novel sites pop before known ones. Never drops or aborts from this — still explores demoted sites. Counter: `PathCounters::deprioritized_known_site`. Do **not** auto-abort from yield-drop counters (`dropped_duplicate_plan` / `signal_contained_plan`). Tried on HARD/LARGER: small `mol_edits` drops on eugenol-MeO / bisMeO-naph / veratrole; **eugenol→allylQ later hits got noisier** (`n_lin_sum` 552→20k, 8-step H₂ detour) — demoting shared early MeO prefers novel detours. Status: not decided — may need walk-prefix / step-class novelty rather than raw site match.
