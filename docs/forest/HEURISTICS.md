...
- **Use yielded plans mid-search (deprioritize / prune).** After a hit is yielded, expand **soft-demotes** hops whose leaf rule + site (unique-edit orbit) already appears in a yielded plan: (1) expand `order_key` sorts those sites later; (2) heap key is `(target_hit, novel_site, seq)` so novel sites pop before known ones. Never drops or aborts from this — still explores demoted sites. Counter: `PathCounters::deprioritized_known_site`. Do **not** auto-abort from yield-drop counters (`dropped_duplicate_plan` / `signal_contained_plan`). Status: not decided — measure HARD multipath (tetraMeO-naph, eugenol-MeO); watch whether demoting shared early MeOs prefers noisy novel detours.
