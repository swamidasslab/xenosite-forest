...
- **Multipath redundancy vs free linearizations.** Yield filter already drops reorderings of the *same* step multiset. Plans that share early dealks but pick **different** OH carbons before quinone DH are different multisets (different sites) — they stay as alternate paths unless site identity is orbit-aware. Status: not decided — next lever is aligning `same_linearizations` with `Step.orbit` / `same_site_class` (automorphism), not denser precedes.
- **Use yielded plans mid-search (deprioritize / prune).** Today `plan_already_yielded` runs only at hit yield. Walks that will only rediscover a covered plan still expand. Candidate: heap-deprioritize (or skip expand) when the walk’s current step multiset (rule + orbit class) is already covered by a yielded plan’s linearizations / is a same_linearizations-compatible prefix of one. Prefer novel step-classes after `target_hit` tier. Must not drop a chemically distinct completion. Status: not decided — needs a predicate + test on HARD multipath (tetraMeO-naph, eugenol-MeO) before enabling.
- **Or of cleavages that share an identical fragment.** Distinct cleaving steps that discard the **same** fragment CSMI (e.g. several O–Me dealks → `OC`) can be one plan node: `Or` of those site/rule arms, not N free ordered Dealks. Shrinks the Deps DAG up front (tetraMeO free-dealk `n_lin` blow-up) and removes search-tree forks that only permute identical sides. **Maybe is linked to the Or choice:** each arm carries (or selects) the cleavage bag for that arm; the discarded fragment is not a free-floating `Maybe` entry independent of which arm fired. This is **not** a revival of archive And/Or plan trees ([DROPPED](DROPPED.md) approved cut — Deps stay the plan language). Schema candidates: `Or` as a site/choice record on a cleaving `Step`, or GraphTree `Or` over arms with `Maybe`/`CleavageSide` on each arm. Status: not decided — name the record before coding; do not add a silent `if` in find_path.
- **Cleavage product graph → Or/Maybe, then non-cleavage across survivors.** Up front (or on demand) enumerate the cleavage layer: edges are cleaving edits; nodes are distinct kept/side CSMI pairs (graph is usually small even on large mols — few distinct leave pieces). From that, fold all paths to each distinct cleavage product into Or + linked Maybe. Non-cleaving chemistry (OH, DH, …) then runs on those kept fragments, not on every interleaved cleave×edit interleaving. **MCS / `atom_diff` still gates:** only cleavage products (and later non-cleavage children) that stay compatible with the target under the existing MCS alignment / closer / site-could-help filters are expanded — the graph does not bypass diff. Complements today's cleavage-first `order_key` (prefer cleaving when target is smaller) with an explicit product DAG rather than another search branch. Status: not decided — measure graph size on HARD cases; schema for the graph + Or fold before coding.
