...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::Match(MatchScoreSpec::log_neg_pc())`.** Heap key is `(score + diversity, score)`, then `seq`. Base score = `Σ −ln(1+child) − ln(1+parent)` over atom+formula (no `max(0,p−c)` flatten). **Diversity** (default on): count accepted pops per key `(rule, site Tags, added-atom Tags)`; primary += `−ln(n+1)`. On pop, recompute; if stale, re-push (`diversity_repush`); else accept and increment. SoftStack ignores. Ablate hard p5: diversity-on bill **1369** vs `no-diversity` **1266** (same drop_dup 13) — breadth costs ~100 bill here; keep on for now. Former **add-both** = `ln` of Product factors kept opt-in. Soft still best hard multipath bill (**1170**). Status: **approved** log-neg-pc; diversity term **not decided** (ablate).
