...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::Match(MatchScoreSpec::log_neg_pc())`.** Heap key is `(score + diversity, score)`, then `seq`. Base score = `Σ −ln(1+child) − ln(1+parent)` over atom+formula (no `max(0,p−c)` flatten). **Diversity** (default on): count accepted pops per key `(rule, site Tags, added-atom Tags)`; primary += `−ln(n+1)`. On pop, recompute; if stale, re-push (`diversity_repush`); else accept and increment. SoftStack ignores. Ablate: `no-diversity`. Hard multipath p5 (pre-diversity): bill **1266** vs add-both/product 1309 / soft 1170. Former **add-both** = `ln` of Product factors (rank-identical to product-both) kept opt-in. Linear `Lin*`: hard p5 tied at 1538 (worse). Status: **approved** log-neg-pc as default (Rust derisk); SoftStack still best hard multipath bill — leave opt-in; diversity term **not decided** (ablate).
