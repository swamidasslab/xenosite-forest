...
- **Metabolite enumerator (stream).** [`enumerate`](../../crates/xenosite-forest/src/enumerate.rs) / [`bfs`](../../crates/xenosite-forest/src/enumerate.rs) / [`dfs`](../../crates/xenosite-forest/src/enumerate.rs): pull-iterator of metabolites up to `max_depth` with accumulated [`PathInfo`](../../crates/xenosite-forest/src/enumerate.rs) (hops = rule, pattern, site Tags, added Tags). BFS queue vs DFS stack (Python `find_path.bfs` / `dfs` shape). CSMI dedup; optional `max_nodes`. Expand via `product_layer` with no target gate. Not find_path; not the product graph materialization. Status: approved (Rust derisk door). Tests: `enumerate` unit.
