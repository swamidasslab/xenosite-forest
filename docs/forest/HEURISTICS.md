...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Key: `(target_hit, novel_site, search_bias, site_progress, cost_gain, seq)` — hits, novel pattern+site, soft scores, parent-relative **cost gain**, then **DFS** `seq` (LIFO among equals). Child cost is computed at enqueue for `cost_gain` (lift, else MCS). **`lazy_closer` default false**: refuse non-closer at enqueue (DFS only among improvers); opt-in lazy still enqueues flat/worse hops for pop-gate. **Measured (HARD, cost_gain + DFS):** tetraMeO-naph 13M→242k; veratrole 371k→84k; eugenol-MeO **1.8M→40M** (local gain into long detours). Eager vs lazy under known `cost_gain`: same `n_lin_sum` (eugenol still 40M). Status: not decided. Tests: `heap_lack_of_improvement_counters_dfs` / `heap_prefers_larger_cost_gain_over_seq` / `hop_cost_gain_is_parent_minus_child`.
