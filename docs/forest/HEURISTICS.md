...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Key: `(target_hit, novel_site, search_bias, site_progress, cost_gain, seq)` — soft scores + parent-relative **cost gain** first. Among equal scores, **`pop_frontier` alternates DFS (max seq) and BFS (min seq)** each real expand/yield (discard/dup does not flip). Child cost at enqueue for `cost_gain`; `lazy_closer` default false. **Measured (HARD, cost_gain + pure DFS):** tetraMeO 242k; veratrole 84k; eugenol **40M**. Remeasure under alternate DFS/BFS. Status: not decided. Tests: `pop_frontier_alternates_dfs_and_bfs` / `heap_lack_of_improvement_counters_dfs` / `hop_cost_gain_is_parent_minus_child`.
