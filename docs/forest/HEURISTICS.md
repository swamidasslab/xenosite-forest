...
- A richer walk-ranking key than target-hit + enqueue order. Status: not decided. The frontier is already a lazy heap (see Already in). **Tried DFS bias:** LIFO `seq` (most recently queued first) instead of FIFO — helps finish one chain of unordered free sites before breadth explores redundant siblings. Sibling cost-sort before enqueue looked like a heap but preferred locally cheap dead ends on PhCH2OH→quinone; do not revive cost as the key without a better one. **Tried `PatternInfo.search_bias`:** soft i8 on the heap after novel-site (higher first). Hydrogenation and OxygenReduction use `-1` (`demote_reductive`): real pathways, but less common and chemically opposite the usual oxidative direction, so prone to undo prior progress (DFS LIFO otherwise dug into H₂/OR first). Soft only — never drops. Status: not decided. When a richer key lands, `_fresh_walk_priority` should recompute it from `atom_diff` / `order_key` / bias data at pop.
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Minimal key: `(target_hit, novel_site, search_bias, seq)` — hits before non-hits, novel pattern+site before known, then **`PatternInfo.search_bias`** (higher first; Hydrogenation + OxygenReduction `-1`), then **LIFO** `seq` (DFS bias: most recently queued pops first). Was FIFO; BFS spread on many unordered free sites explored redundant breadth. **Measured (DFS):** tetraMeO-naph bill 957→111 edits. H₂ demotion alone: veratrole `n_lin` 6.3B→189k, tetraMeO-naph 240M→968k. Adding OR demotion: eugenol-MeO first hit becomes the short 5-step plan (`n_lin=60`), but tetraMeO-naph bill/n_lin regress (111→196 edits, 968k→13M). Status: not decided whether LIFO / which demotions stay. On pop, cheaply rescore; if the fresh key differs and is worse than peek, `heappush` back instead of expanding (compare-to-peek; no O(n) resort). Not a forest ranker. Test: `test_stale_priority_is_pushed_back` / `heap_prefers_most_recently_queued_among_peers` / `heap_prefers_higher_search_bias_over_seq`.
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference in `find_path` after novel-site: higher pops sooner on the max-heap. Negative demotes. **Hydrogenation** and **OxygenReduction** use `-1` (shared `demote_reductive`): real but less-common / counter-oxidative — prone to undo other edits under DFS LIFO — not invalid. Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; this is a score knob, not a filter. Tests: `heap_prefers_higher_search_bias_over_seq` / `reductive_patterns_carry_negative_search_bias`. Measured: H₂ demotion alone cut veratrole / tetraMeO-naph `n_lin` by orders under DFS; OR demotion targets remaining eugenol-MeO / veratrole detours.
