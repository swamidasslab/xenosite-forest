...
- **Fresh `atom_diff` alignment is bare MCS by default.** [`mcs_extend`](../../crates/xenosite-forest/src/atom_diff.rs) (MCS then placeable grow) remains available for lift+extend seeds and as an opt-in on every expand (`USE_MCS_EXTEND` / `find_path_bench --mcs-extend`). Using extend on every `atom_diff` with PhaseOne+EpoxideHydration regressed hard filter bill **273→449** (veratrole 60→216); bare MCS restores 273. Rematch uses the same door as expand (do not mix bare expand with extend rematch). Status: **approved** bare default; full-extend default **not approved** (ablate). Tests: `epoxide_o_extends_onto_target_hydroxyl` / `mcs_align_respects_use_mcs_extend_flag` / `epoxide_toward_diol_drops_with_heavier_n_extra`.
