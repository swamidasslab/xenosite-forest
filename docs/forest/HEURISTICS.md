...
- **find_path ablation (`find_path_ablate`).** One-at-a-time vs baseline (eager, **add-both** = log-Product, skeleton on, PhaseOne). Mid+hard, paths=1 and paths=5. No misses. Novel-site / `target_hit` heap tiers **removed** (DROPPED). Soft wins hard p5, loses hard p1 — keep SoftStack opt-in.
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference under **`HeapScoreMode::SoftStack`** (not used by default `Match(add-both)`). Higher pops sooner; negative demotes. **Hydrogenation** and **OxygenReduction** use `-1` (`demote_reductive`). Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; SoftStack-only score knob. Tests: `heap_prefers_higher_search_bias_over_seq` / `reductive_patterns_carry_negative_search_bias`.
