...
- **Use yielded plans mid-search (deprioritize / prune).** After a hit is yielded, expand **soft-demotes** hops whose **pattern name + site** (unique-edit orbit on `PathStep`) already appears in a yielded path — not bare site, not leaf rule alone. (1) expand `order_key` sorts those later; (2) heap key is `(target_hit, novel_site, seq)`. Never drops or aborts. Counter: `PathCounters::deprioritized_known_site`. Do **not** auto-abort from yield-drop counters. Earlier rule+site key over-demoted (eugenol later hits noisier). Status: not decided — remeasure HARD with pattern+site.
