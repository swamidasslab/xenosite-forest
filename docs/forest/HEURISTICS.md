...
- **Lift child MCS via extend (MCS when not cost 0).** After an edit, tag-lift the parent MCS onto the product and **always extend** where the gap is placeable (unmapped product heavies onto free same-element target atoms bonded to mapped neighbor images — same rules as add-placement / `n_extra`). **Keep the lift only if `field_cost == 0`** (guaranteed — skip MCS). **Otherwise do not Aut-chase or trust a non-zero lift — use a fresh MCS** and count `PathCounters::mcs_lift_rematch`. Tag-lift impossible → `mcs_lift_fallback` (expect zero). No double work of Aut-then-MCS. Correctness suite: `lift_mcs_correctness`. Status: approved (Rust derisk).
...
- A richer walk-ranking key than target-hit + enqueue order. Status: not decided. Default is **`Match(product-both)`** (formula × atom_diff improve×close); SoftStack and other `MatchScoreSpec` recipes remain opt-in (`--score`, `--matrix`). Plain max-heap pop. Sibling absolute-cost sort before enqueue preferred locally cheap dead ends on PhCH2OH→quinone — do not revive that. **Tried `PatternInfo.search_bias`:** SoftStack-only soft i8. **Dropped:** alternate DFS/BFS among equal scores. Matrix: close-atom lowest hard bill — switch not decided.
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current — aromatic |Δ| in atom cost; no special H.** `field_cost = 3·(|cleaved|+n_extra+|cleavage_bonds|) + |aromatic_delta|`. Per mapped atom, aromatic is 0/1; mismatch (either direction) joins `aromatic_delta` and costs 1. `loses_aromaticity` stays the filter set (reactant aromatic → target not). No cached `h_delta` / `h_loss` / `h_gain` — methods `AtomDiff::atom_h_delta` / `h_loss` / `h_gain` recompute from MCS mapping + mols (same idea as N1 oxygen). `formula_l1` counts H like any element. Soft bond-order stay filter-only. Dropped cached `needs_oxygen` / carbonyl / alcohol (recompute N1). Status: not decided (aromatic-in-cost trial; was filter-only).
...
- **Epoxidation / EpoxideHydration `dearomatizes` capability.** Catalog `Effect.dearomatizes=true` (aromatic epoxidation / diol look-ahead clears the ring bit at the site). [`PatternInfo::resolve_for_match`](../../crates/xenosite-forest/src/pattern.rs) resolves false on aliphatic site_map atoms (context mol, not Kekulé form). Same resolve shape as Hydrogenation / pair `merge_dearomatizes`. Status: approved (Rust derisk). Tests: `epoxidation_dearomatizes_capability_resolves_on_aromatic_site` / `epoxide_hydration_pattern_info_and_plan` / always-on [`pattern_info_catalog`](../../crates/xenosite-forest/tests/pattern_info_catalog.rs).
- **Always-on PatternInfo catalog audit.** `cargo test -p xenosite-forest --test pattern_info_catalog` (also under plain `cargo test -p xenosite-forest`): every `catalog_names` leaf — including `EpoxideHydration` — unique names, sealed `delta_formula`, site_map/kind shape, edit present; chemistry probe that kept-site aromatic→non-aromatic edits declare `dearomatizes` capability; resolve check on aromatic vs aliphatic probes. Status: approved (Rust derisk).

- **find_path ablation (`find_path_ablate`).** One-at-a-time vs baseline (eager, product-both, novel on, skeleton on, PhaseOne). Mid+hard, paths=1 and paths=5. No misses in any variant.
  ...
  Read: single-path (p1) novel/skeleton are inert. SoftStack hurts hard p1, wins hard p5 (tetraMeO-naph). Turning **off** novel-site demotion cuts hard p5 bill almost in half — demotion is spending work. Skeleton-off also lowers bill but fills `max_paths` with remapped twins (more hits, fewer `drop_dup`) — speed via redundancy, not better search. EpoxideHydration taxes every suite (~20–250 bill) without helping these quinone/dealk cases (diol look-ahead not exercised). Status: not decided — proposals below.

  **Propose drop / simplify (clarity, no miss risk on these benches):**
  1. **novel_site soft-demote** (`deprioritize_known_site`) — candidate to default **off** or remove. HEURISTICS already “not decided”; p5 hard is strictly worse with it on. Counter + expand sort + heap tier are complexity for a soft demote that burns bill.
  2. **SoftStack as live path** — keep opt-in only; do not revive as default. Single-path hard regresses; multipath hard win is score choice, not a reason to keep SoftStack lex complexity in the hot path.
  3. **lazy_closer** — leave default eager; lazy is near-neutral. Not worth a clarity cut.
  4. **skeleton twin yield-drop** — **keep**. Off looks “cheaper” only because twin hits pad `max_paths` and stop early; exact `same_linearizations` alone is not enough for remapped free steps.
  5. **EpoxideHydration in PhaseOne** — **keep for now** (this leaf’s point is the diol look-ahead). Record the tax; do not drop from PhaseOne without a diol-bench that needs it. Peers Epoxidation+Opening already cover the two-hop path.
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference under **`HeapScoreMode::SoftStack`** after novel-site (not used by default `Match(product-both)`). Higher pops sooner; negative demotes. **Hydrogenation** and **OxygenReduction** use `-1` (`demote_reductive`). Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; SoftStack-only score knob. Tests: `heap_prefers_higher_search_bias_over_seq` / `reductive_patterns_carry_negative_search_bias`.
