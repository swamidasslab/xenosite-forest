...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `MoleculeShells` / `AlignedShells` (n0/n1/n2).** Every heavy atom stores `aromatic` plus element bags `n0`/`n1`/`n2` that **include H** (ethane C: `n0=C:1`, `n1=C:1 H:3`, `n2=H:3`). Align → deltas + `unaligned_*`. **Cost** = iterate the structure (aromatic + n0, n1, n2) and Σ|δ| with missing keys as 0 (`shell_l1`). Site selection: that cost on `at_sites`; product closeness: same on the full align. No category enum. Example: `eval_site_shells` / `show_matched_atoms`. Status: not decided.
