...
- A richer walk-ranking key than target-hit + enqueue order. Status: not decided. Default is **`MatchProduct`** (formula × atom_diff improve×close); SoftStack remains opt-in. Plain max-heap pop. Sibling absolute-cost sort before enqueue preferred locally cheap dead ends on PhCH2OH→quinone — do not revive that. **Tried `PatternInfo.search_bias`:** SoftStack-only soft i8. **Dropped:** alternate DFS/BFS among equal scores.
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::MatchProduct`.** After hard tiers `target_hit` then `novel_site`, one product score across **formula** (heavy L1 vs target) and **atom_diff** cost: `improvement × closeness` with `improvement = (max(0, Δformula)+1) × (max(0, Δatom)+1)` and `closeness = (SCALE/(1+f_child)) × (SCALE/(1+a_child))`. Plain [`BinaryHeap::pop`] (best Ord; `seq` tiebreak only). Bench: `find_path_bench --score match` (default). **Measured vs SoftStack (filter-only, plain pop):** mid tied; hard TOTAL ~1.20s vs ~1.30s, tetraMeO-naph bill 96 vs 127; most hard rows lower bill. Status: approved (Rust derisk).
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference under **`HeapScoreMode::SoftStack`** after novel-site (not used by default `MatchProduct`). Higher pops sooner; negative demotes. **Hydrogenation** and **OxygenReduction** use `-1` (`demote_reductive`). Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; SoftStack-only score knob. Tests: `heap_prefers_higher_search_bias_over_seq` / `reductive_patterns_carry_negative_search_bias`.
