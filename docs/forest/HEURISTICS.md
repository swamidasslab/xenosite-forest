...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Minimal key: `(target_hit, novel_site, search_bias, seq)` — hits before non-hits, novel pattern+site before known, then **`PatternInfo.search_bias`** (higher first; Hydrogenation `-1`), then **LIFO** `seq` (DFS bias: most recently queued pops first). Was FIFO; BFS spread on many unordered free sites explored redundant breadth. **Measured:** tetraMeO-naph bill 957→111 edits / 2.2s→0.27s with LIFO alone, but eugenol-MeO / veratrole later hits got much noisier (`n_lin_sum` exploded; deep H₂/OR detours first). Status: not decided whether LIFO / bias stay — trade bill vs plan quality. On pop, cheaply rescore; if the fresh key differs and is worse than peek, `heappush` back instead of expanding (compare-to-peek; no O(n) resort). Not a forest ranker. Test: `test_stale_priority_is_pushed_back` / `heap_prefers_most_recently_queued_among_peers` / `heap_prefers_higher_search_bias_over_seq`.
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference in `find_path` after novel-site: higher pops sooner on the max-heap. Negative demotes. Hydrogenation patterns use `-1` because H₂ is a real but less-common pathway and runs opposite the usual oxidative direction — prone to undo other edits — not because it is invalid. Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; this is a score knob, not a filter. Tests: `heap_prefers_higher_search_bias_over_seq`.
