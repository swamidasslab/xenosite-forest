...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved. Minimal key: `(0 if target_hit else 1, seq)` — hits before non-hits, then **LIFO** `seq` (DFS bias: most recently queued pops first). Was FIFO; BFS spread on many unordered free sites explored redundant breadth. On pop, cheaply rescore; if the fresh key differs and is worse than peek, `heappush` back instead of expanding (compare-to-peek; no O(n) resort). Not a forest ranker. Test: `test_stale_priority_is_pushed_back`.
