"""Parity: archived AtomTracker vs deprecated facade (xf proxy)."""
...
from xenosite._archive_forest.base import AtomTracker as ForestAtomTracker
from xenosite._archive_forest.rules import Hydroxylation as ForestHydroxylation
from xenosite.forest.atom_tracker import AtomTracker as LiveAtomTracker
from xenosite.forest.rules import Hydroxylation as LiveHydroxylation
...
def test_topol_equiv_parity_on_same_smiles():
    ...
    live_mol = Chem.MolFromSmiles(smi)
    assert forest_mol is not None and live_mol is not None
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
        assert ForestAtomTracker.topol_equiv(forest_mol) == LiveAtomTracker.topol_equiv(
            live_mol
        )
...
def test_initialize_tags_depths_parity():
    ...
    live_mol = Chem.MolFromSmiles("CCO")
    assert forest_mol is not None and live_mol is not None
    ForestAtomTracker().initialize_tags(forest_mol)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
        LiveAtomTracker().initialize_tags(live_mol)
        assert (
            ForestAtomTracker.depths(forest_mol)
            == LiveAtomTracker.depths(live_mol)
            == [0]
        )
        f_tags = ForestAtomTracker.tags(forest_mol)
        p_tags = LiveAtomTracker.tags(live_mol)
...
def test_hydroxylation_product_depths_and_added_atom_parity():
    ...
    live_parent = Chem.MolFromSmiles("CCO")
    ...
    with warnings.catch_warnings():
        ...
        p_depths = LiveAtomTracker.depths(live_product)
        assert f_depths == p_depths == [0, 1]

        f_tags = ForestAtomTracker.tags(forest_product)
        p_tags = LiveAtomTracker.tags(live_product)
...
def test_live_tags_strict_untraced_raises():
    mol = Chem.MolFromSmiles("CC")
    assert mol is not None
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
        with pytest.raises(KeyError):
            LiveAtomTracker.tags(mol, strict=True)
        assert LiveAtomTracker.tags(mol, strict=False) == {}
