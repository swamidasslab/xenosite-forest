...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `HeapScoreMode::MatchProduct`.** Same hard tiers, then one product score across **formula** (heavy L1 vs target) and **atom_diff** cost: `improvement × closeness` with `improvement = (max(0, Δformula)+1) × (max(0, Δatom)+1)` and `closeness = (SCALE/(1+f_child)) × (SCALE/(1+a_child))`. Bench: `find_path_bench --score match`. Status: not decided.
