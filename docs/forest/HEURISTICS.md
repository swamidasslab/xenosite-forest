...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `MoleculeShells` / `AlignedShells` (n0/n1/n2).** Every heavy atom stores `aromatic` plus element bags `n0`/`n1`/`n2` that **include H**: center H in `n1`, H on dist-1 heavies in `n2` (ethane C: `n0=C:1`, `n1=C:1 H:3`, `n2=H:3`). Two molecules + alignment → same shape as **deltas**, plus `unaligned_reactant` / `unaligned_target`. Roles: (1) **site selection** — site-scoped shells (`AlignedShells::at_sites` / optional `site_delta_forecast`); alcohol vs carbonyl is in the site dicts (`O:+1 H:−1` vs `O:+1 H:−2`), so a molecule-wide cost is not required for the gate; (2) **product closeness** — full-align `AlignedShells::cost` = Σ|δ| over aromatic + n0/n1/n2 (still needs the full diff). Example: `show_matched_atoms`. Status: not decided.
