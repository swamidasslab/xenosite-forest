...
impl AtomDiff {
    ...
    fn field_cost(&self) -> usize {
        // MCS heavy-atom alignment + per-atom H counts (`h_delta`). Global H
        // is also in `formula_l1` for the formula heap metric; atom metric needs
        // the local map. Unmapped target O is in `n_extra`.
