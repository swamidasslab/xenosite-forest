...
- **Cleavage product graph → Or/Maybe, then non-cleavage across survivors.** Enumerate the cleavage layer (on demand / BFS); both fragment CSMIs are graph nodes. Fold arms by fragment multiset into Or. **MCS / `atom_diff` still gates** which fragments enqueue. Expand gate: strict cost drop vs parent **and** ha ≥ target; if **both** sides match, **both** stay — no single-winner prune. Shrink diffs **tag-lift** from the parent MCS when that already shows a cost drop (`atom_diff_after_cleavage`); else one MCS. **First-pass (tried):** `FindPathConfig.cleavage_first` — inside `find_path`'s site-expansion loop (no depth cap): if this expand still has cleaving emissions, take cleaving only; when expand has none, non-cleavage on that core and exclude later cleaves. Presence in the emission list is the gate — not a depth counter. Collapses dealk↔OH interleaving (redundancy), not a wall-time knob. Default **off**. Status: not decided.
