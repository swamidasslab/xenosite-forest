...
- A richer walk-ranking key than target-hit + enqueue order. Status: not decided. The frontier is already a lazy heap (see Already in). **Heap shape:** soft scores first (`search_bias`, site H-progress, `cost_gain`), then `seq` tiebreak; **plain max-heap pop** (no DFS/BFS alternation). Sibling absolute-cost sort before enqueue preferred locally cheap dead ends on PhCH2OH→quinone — do not revive that; `cost_gain` is parent-relative only. **Tried `PatternInfo.search_bias`:** soft i8 after novel-site. Hydrogenation and OxygenReduction use `-1` (`demote_reductive`). Soft only — never drops. Status: not decided. When a richer key lands, `_fresh_walk_priority` should recompute it from `atom_diff` / `order_key` / bias data at pop. **Dropped:** alternate DFS/BFS among equal scores (was measured for HARD multipath; plain Ord pop is the rule now).
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::SoftStack`.** After hard tiers `target_hit` then `novel_site` (vs already-yielded plans), soft lexicographic max-heap key:

  1. `search_bias` ([`PatternInfo::search_bias`]; e.g. Hydrogenation / OxygenReduction −1)
  2. `site_progress` (site H-progress vs parent `AtomDiff`, best one MCS placement)
  3. `cost_gain` = parent atom_diff cost − child cost (≤0 counters DFS vs positive peers)
  4. `seq` tiebreak only (higher preferred). **Plain [`BinaryHeap::pop`]** — best Ord value; no DFS/BFS band alternation.

  Child cost at enqueue for `cost_gain`; `lazy_closer` default false. **`delta_formula` is not a heap score** — soft mismatch warn/counter only. Status: not decided for the soft stack vs simpler alternatives. Tests: `heap_pops_best_ord_value_only` / `heap_lack_of_improvement_counters_dfs` / `hop_cost_gain_is_parent_minus_child`.

  **Trial — `HeapScoreMode::MatchProduct`.** Same hard tiers, then one product score across **formula** (heavy L1 vs target) and **atom_diff** cost: `improvement × closeness` with `improvement = (max(0, Δformula)+1) × (max(0, Δatom)+1)` and `closeness = (SCALE/(1+f_child)) × (SCALE/(1+a_child))`. Bench: `find_path_bench --score match`. **Measured (filter-only):** mid TOTAL 0.043s vs SoftStack 0.047s (MeOPhOH bill 22 vs 31); hard TOTAL ~1.19s vs ~1.33s (tetraMeO-naph bill 96 vs 117; all hard rows lower bill). Status: not decided.
