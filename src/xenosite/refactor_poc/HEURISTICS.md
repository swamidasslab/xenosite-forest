...
- A priority heap instead of the queue. Order transforms by how likely they are to help, and let that priority change after a path has already been found. Partial: a child that is already the target is prepended and further site edits on that node stop once `max_paths` hits are queued (TBA).
