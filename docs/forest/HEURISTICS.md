...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `MoleculeShells` / `AlignedShells` (n0/n1/n2).** Every heavy atom stores `aromatic` plus element bags `n0`/`n1`/`n2` that **include H** (ethane C: `n0=C:1`, `n1=C:1 H:3`, `n2=H:3`). Align → deltas + `unaligned_*`. **Site cost** = Σ over site atoms of normalized shell distance between `current + δ` and target (`site_shell_cost`). Each shell is `L1 / Σ max(|a|,|b|)` ∈ [0,1] (`shell_norm_l1`), so per-atom ≤ 3 (n0+n1+n2); aligned atoms share n0 → ≤ 2. Site atoms matched as a multiset (orbit/MCS safe). δ = 0 is distance now; a complete edit scores 0. Close pairs use the **joint** site atom list. **Site bag** (`SiteShellBag`) is the order-invariant multiset of kept deltas + cleaved/added for forecast↔applied checks (`check_site_shell_bags` warn/error); methyl leave is cleaved. No category enum. Status: not decided.
