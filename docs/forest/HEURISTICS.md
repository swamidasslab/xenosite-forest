...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference in `find_path` after novel-site: higher pops sooner on the max-heap. Negative demotes. Hydrogenation patterns use `-1` to blunt DFS LIFO digging into H₂ first. Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; this is a score knob, not a filter. Tests: `heap_prefers_higher_search_bias_over_seq`.
