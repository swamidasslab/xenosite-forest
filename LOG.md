...
- **0.7.3 release.** Both-methide, keep dealkylation fragments, hydrolysis bond site, drop two-double nitrogen and re-aromatized quinone systems. find_path ethane site stays `{1}` by default; same topological class as metabolize `{0}` — use `canonical_emitted_sites=True` for the lex representative.

- **Alprazolam's aromatic cation is not a quinone.** Site `{1, 4}` on `Cc1nnc2n1-c1ccc(Cl)cc1C(c1ccccc1)=NC2` emitted `Cc1nnc2cnc(-c3ccccc3)c3cc(Cl)ccc3[n+]1-2`. The system that was kekulized is aromatic again and has no localized double bond. That emission is dropped. Other aromatic rings may stay. A quinone, quinone-imine, or quinodimethane keeps a non-aromatic double bond, so those stay.

- **Aspirin hydrolysis site is the cleaved bond on the parent.** `Hydrolysis.site_kind` is `bond` and `site_map` is `(2, 3)`. Both fragments of `CC(=O)Oc1ccccc1C(=O)O` → `CC(=O)O` + `O=C(O)c1ccccc1O` record `{1, 3}`. The indexes are the parent’s ester bond, not renumbered onto either piece. It was the carbonyl carbon `{1}`.

- **`C=[N+]=C` is not a quinone.** Carbamazepine `NC(=O)N1c2ccccc2C=Cc2ccccc21` site `{4, 17}` paired iminium on one ring carbon with dealkylation of the carboxamide on the other. The nitrogen lost its third substituent and kept both double bonds: `C1=c2ccccc2=[N+]=c2ccccc2=C1` plus `NC=O`. RDKit sanitizes that cation. It is not an iminium (one double bond, two single bonds). `_sanitize_piece` now drops a nitrogen with two double bonds. Chlorpromazine's charged quinone-imine stays. The other `[n+]` rows are not charge failures.

- **The four remaining quinone products were the dealkylation fragment.** `sanitized_fragments` was dropping the piece marked `dealk-noncarbon` whenever another piece existed. Keeping every fragment emits `CN1C(C=O)=C(O)c2ccccc2S1(=O)=O`, `CC(C)N=C(N)N=CN`, `O=CCCc1ccccc1`, and `O=CCOc1ccccc1`. The ring-open imidazopyridine golden is still present.

- **Both methide ends resolve.** The pair skip `if both methide: continue` was wrong. Xylene `Cc1ccccc1C` now emits `C=c1ccccc1=C`. Of the 27 archive quinone products live was missing, 23 are back. The other 4 are `dealk`+`addO` and still absent. There is no separate "no methides" mode. `merge_effects` sets `methide` when either end has it.

- **The 27 live-missing quinone products are odd-bond paths.** Archive walk for every product in the status table: 24 odd bonds with both ends in a 6-ring, 3 odd bonds with `both6=False`. None are even. Live `QuinoneFormation.metabolites` does not contain them before `sanitized_fragments`. Archive edits: 23 are `single2double` + `single2double` (methide / methylenedioxy), 4 are `dealk`+`single2double` plus `addO` (the aldehyde and guanidine cleavages). Reply: `agent-exchange/from-forest/2026-09-21-quinone-27.md`.

- **Predict regression list, current tree.** Commit `21b2306`, dirty (`rules.py`, `rdkitutil.py`, quinone golden test, this log). `QuinoneFormation().metabolize` vs `agent-exchange/from-predict/2026-09-21-regressions.md`: 0/83 expected products present. Archive emits 27 of those 83; 56 are in neither library. The four `[n+]` reactants are still charged here, and archive emits the same charged SMILES. Dealkylation does not repeat an identical `(site, both fragments)` row (`CN(C)Cc1ccccc1` is 20/20 unique; `CCC` is 4/4). The repeated carbon fragment is a different oxygenation pattern. Hydrolysis on aspirin is still site `{1}`, not bond `{1, 3}`. `find_path('CC','CCO')` with filters reports Hydroxylation `{1}` because `atom_diff` maps the new oxygen onto carbon 1; unfiltered search and `metabolize` both report `{0}`. Not a 1-based index. Reply: `agent-exchange/from-forest/2026-09-21-regression-status.md`.

## 2026-09-20

- **Quinone ring-open goldens were dropped after the edit.** For `CS(=O)(=O)c1ccc(-c2cn3ccccc3n2)cc1`, archive `QuinoneFormation` has 14 products including the three neutral 9-ring goldens. The edit is dealkylation of the bridgehead N–C plus carbonyl on the neighbor (`pair (11, 12)`, path length 1). Live already made that graph, then `sanitized_fragments` discarded the only piece because the cleaved atom still carried `dealk-noncarbon`. A ring opening is not a leaving group; the marked atom is dropped only when cleavage produces another piece. `edit_iminium` was also ignoring `skip_same_rings`, which charged the bridgehead and emitted `C2=Nc3cccc[n+]3C2=O`. After both fixes the 14 archive products match and that `[n+]` is gone. Live still has 3 extra ring-opened SMILES archive does not. Two saturated downstream goldens (`C2NC3CCC(=O)CN3C2=O` and `C2NC3C(=O)CCCN3C2=O`) are in neither library.

- **Epoxidation uses supplier kekulé mols again.** SMARTS is `[#6:1]=[#6,#7:2]`, not `=,:`. `Epoxidation.metabolites` runs that SMARTS on each `ResonanceMolSupplier(..., KEKULE_ALL)` mol. Bond orders are not laid back onto the aromatic parent, and the cached conjugated-component parent is not used. Unique-edit `seen` is shared across those forms; `incident_orders` stay on the aromatic context. Other `ResonanceRule`s still use the cached parent: walking supplier forms dropped anisole and pyridine ring-open products that parity requires. Six neutral goldens are in `tests/forest/test_epoxidation_golden_products.py`. `pytest tests/forest`: 1626 passed, 1 skipped, 2 xfailed. Quinone was still open at this point; the entry above is the follow-up.
