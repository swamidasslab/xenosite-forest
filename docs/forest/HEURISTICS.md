...
- **Topo-id filter then promote (Rust tagged lift).** Best shape: filter sites on topological id, then pick one concrete atom from the passing group to edit — unique-edit narrowing without editing the wrong orbit rep before filters. **Today (interim):** emit every SMARTS hit; atom-diff filters match concrete MCS atom ids (no `expand_by_rank`); product/`seen` csmi collapses yields. Status: not decided for promote; interim is for tagged MCS lift honesty.
