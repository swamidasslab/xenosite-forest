...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::Match(MatchScoreSpec::product_both())`.** Heap key is score then `seq` only — no `target_hit` / `novel_site` bool tiers and no hit sentinel in the score. A target hit is just atom_diff / formula distance 0 (closeness max). Match-family score from data `combine × metric` (not a SoftStack lex key). Default recipe is **product × both**: `improvement × closeness` with `formula_l1` (includes H) and atom_diff cost. Improvement = max(0, parent − child) + 1; closeness = `SCALE / (1 + child)`; both-metrics multiply factors. Plain [`BinaryHeap::pop`]. Bench: `find_path_bench --score product-both` (aliases `match` / `product`); `--matrix` runs all 9 + soft. Status: approved (Rust derisk) as default pending matrix follow-up.
...
- **find_path ablation (`find_path_ablate`).** One-at-a-time vs baseline (eager, product-both, skeleton on, PhaseOne). Mid+hard, paths=1 and paths=5. No misses in any variant. Novel-site demotion and `target_hit` heap tier **removed** (DROPPED); score has no hit sentinel — hits are cost/formula 0.
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference under **`HeapScoreMode::SoftStack`** (not used by default `Match(product-both)`). Higher pops sooner; negative demotes. **Hydrogenation** and **OxygenReduction** use `-1` (`demote_reductive`). Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; SoftStack-only score knob. Tests: `heap_prefers_higher_search_bias_over_seq` / `reductive_patterns_carry_negative_search_bias`.
