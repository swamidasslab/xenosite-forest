...
- **Cleavage product graph → Or/Maybe, then non-cleavage across survivors.** Enumerate the cleavage layer (on demand / BFS); both fragment CSMIs are graph nodes. Fold arms by fragment multiset into Or. Non-cleaving chemistry runs across those nodes. **MCS / `atom_diff` still gates** which fragments enqueue. Expand gate (tried): strict cost drop vs parent **and** fragment heavy-atom count ≥ target; if **both** sides match, **both** enqueue — no single-winner prune. Non-matching sides stay on the Or for Maybe. Status: not decided for search integration; measure with `cleavage_graph_bench`.
