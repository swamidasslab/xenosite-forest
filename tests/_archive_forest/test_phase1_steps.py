...
from xenosite._archive_forest import AtomRef, StepPlan
from xenosite._archive_forest.base import ConjugatedSystems
...
from xenosite._archive_forest.step_plan import And, Or, Step, StepPlan
from xenosite._archive_forest.utils import canon_smi, refresh_mol, unmapped_smiles
...
def _hit_query_modifications(rule, mol) -> frozenset[str]:
    ...
    for matches in rule.match_queries(template).values():
        for _maps, mods, *_rest in matches:
            hit |= _mod_names(mods)
...
def test_quinone_smarts_probe_phase1_steps(probe_id, smiles):
    ...
    for site, products in qf.metabolize(
        ...
        for product in products:
            ...
            assert plan in public.branches()
...
def test_quinone_stepplan_full_forest_apply_apap():
    ...
    plan = next(
        b
        for b in QuinoneFormation().phase1_steps(mol, frozenset({4, 7})).branches()
        if len(b) == 1
    )
...
def test_epoxidation_degenerate_phase1_steps():
    ...
    plan = Epoxidation().phase1_steps(mol, frozenset({0, 1}))
    assert plan == StepPlan.singleton("Epoxidation", frozenset({0, 1}))
    assert list(plan.iter_linearizations()) == [plan.steps]
...
def test_ndealkylation_degenerate_phase1_steps():
    ...
    plan = NDealkylation().phase1_steps(mol, site[1])
    assert plan == StepPlan.singleton("NDealkylation", frozenset(site[1]))
...
def test_ndealkylation_attach_matches_public():
    ...
    assert public and stamped == public


def test_hydroxylation_degenerate():
    mol = MolFromSmiles("CC")
    site, _ = next(Hydroxylation().metabolize(mol, tag_atoms=False))
    plan = Hydroxylation().phase1_steps(mol, site[1])
    assert plan == StepPlan.singleton("Hydroxylation", frozenset(site[1]))


def test_quinone_apap_phase1_steps_single_dh():
    mol = MolFromSmiles("CC(=O)Nc1ccc(O)cc1")
    plan = QuinoneFormation().phase1_steps(mol, frozenset({4, 7}))
    assert plan
    # Prefer the single2double+single2double plan: only Dehydrogenation on N and O.
    dh_only = [
        b
        for b in plan.branches()
        if len(b) == 1 and b.steps[0].rule == "Dehydrogenation"
    ]
    assert dh_only
...
def test_quinone_benzene_addo_layers():
    ...
    plan = QuinoneFormation().phase1_steps(mol, frozenset({0, 3}))
    assert plan
    layered = [b for b in plan.branches() if len(b) == 3]
    assert layered
    branch = layered[0]
    assert isinstance(branch, StepPlan)
    assert type(branch) is StepPlan  # Seq spine: And(preps) → DH
    orders = list(branch.iter_linearizations())
    assert len(orders) == 2
...
def test_quinone_attach_phase1_steps_matches_public():
    ...
    assert stamped[0] in public.branches()
...
def test_quinone_attach_phase1_steps_survives_tagging():
    ...
    assert stamped
    assert StepPlan.from_mol(stamped[0]) in qf.phase1_steps(
        mol, frozenset({4, 7})
    ).branches()
...
def test_nested_or_and_seq_linearizations_and_json():
    """Nested Or(And(...), StepPlan(...)) linearizations + JSON round-trip."""
    a = Step("Hydroxylation", frozenset({0}))
    b = Step("Hydroxylation", frozenset({1}))
    c = Step("Dehydrogenation", frozenset({0, 1}))
    d = Step("Epoxidation", frozenset({2, 3}))
    plan = Or([And([a, b]), StepPlan([c, d])])
    lins = list(plan.linearizations())
    # And(a,b): 2 orders; StepPlan(c→d): 1 order
    assert len(lins) == 3
    assert "|" in str(plan)
    roundtrip = StepPlan.from_json(plan.to_json())
    assert roundtrip == plan
    assert list(roundtrip.linearizations()) == lins
    # Efficient membership matches full expansion
    for lin in lins:
        assert plan.contains(lin.steps)
        assert lin.steps in plan
    assert not plan.contains((c, a))
    assert plan.contains(["Hydroxylation", "Hydroxylation"], by="rule")
    assert plan.contains(["Dehydrogenation", "Epoxidation"], by="rule")
    assert not plan.contains(["Epoxidation", "Dehydrogenation"], by="rule")


def test_stepplan_contains_avoids_full_expansion():
    """And of many leaves: contains is O(partitions), not n! materialization."""
    steps = [Step("Hydroxylation", frozenset({i})) for i in range(5)]
    plan = And(steps)
    recipe = tuple(steps[::-1])  # one of 5! orders
    assert plan.contains(recipe)
    assert not plan.contains(recipe[:-1] + (Step("Dehydrogenation", frozenset({0})),))



def test_quinone_plan_str_shows_or_between_routes():
    mol = MolFromSmiles("CC(=O)Nc1ccc(O)cc1")
    plan = QuinoneFormation().phase1_steps(mol, frozenset({4, 7}))
    if len(plan.branches()) > 1:
        assert "|" in str(plan)
    # All linearizations are concrete Step sequences
    assert list(plan.linearizations())
