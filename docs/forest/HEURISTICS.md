...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `MoleculeShells` / `AlignedShells` (n0/n1/n2).** Every heavy atom stores `aromatic` plus element bags `n0`/`n1`/`n2` that **include H**. Align → deltas + `unaligned_*`. **Gate bench baseline** (best so far on mid/hard `cost>0` vs live gate): legacy `at_sites` Σ|δ| on kept atoms only (no cleaved count) — mid P/R/A 0.07/1.00/0.48, hard 0.05/1.00/0.22. Greedy ablation from that baseline: **+leave, +cleaved count, multiset norm shells, multiset norm+dearomatic — none improved** (all lowered mid agree). **Edit residual** `Σ|projected−target|` with `projected = current + δ`, δ = product−reactant (not reactant−product); keep when residual drops vs δ=0 — first clear win (mid agree ~0.68, hard ~0.92). Cost compares projected to target; wrong-sign δ fails that landing. `site_shell_cost` / `SiteShellBag` / leave expand for residual / bag checks. Status: not decided.
