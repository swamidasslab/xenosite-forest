...
- **High `n_lin` is not fixed by inventing precedes.** `Deps::bind` only adds edges from `WillAdd` / `AddedBy` (e.g. OH ≺ DH that consumes that oxygen). Independent Dealkylations of distinct methoxys are chemically unordered — 8 steps with one OH≺DH edge → `n_lin = 8!/2 = 20160` is the free DAG, not a miss. Fake precedes between free dealks would lie. Status: approved (do not invent order).
...
- **Use yielded plans mid-search (deprioritize / prune).** Today `plan_already_yielded` runs only at hit yield. Walks that will only rediscover a covered plan still expand. Candidate: heap-deprioritize (or skip expand) when the walk’s current step multiset is already covered by a yielded plan’s linearizations / is a same_linearizations-compatible prefix of one. Prefer novel step-classes after `target_hit` tier. Must not drop a chemically distinct completion. Status: not decided — needs a predicate + test on HARD multipath (tetraMeO-naph, eugenol-MeO) before enabling.
- **Or of cleavages that share an identical fragment.** Distinct cleaving steps that discard the **same** fragment CSMI multiset fold into one [`CleavageOr`](../../crates/xenosite-forest/src/cleavage_graph.rs): arms share sorted `fragments` (both sides of the bifurcation are first-class). Choosing an arm **and** a continuation fragment builds linked [`Maybe`](../../crates/xenosite-forest/src/canonical_plan.rs) from the other side(s). Not a revival of archive And/Or plan trees ([DROPPED](DROPPED.md)). Status: not decided for find_path wiring; schema + one-hop/BFS builder landed in `cleavage_graph`.
- **Cleavage product graph → Or/Maybe, then non-cleavage across survivors.** Enumerate the cleavage layer (on demand / BFS); both fragment CSMIs are graph nodes. Fold arms by fragment multiset into Or. **MCS / `atom_diff` still gates** which fragments enqueue. Expand gate: strict cost drop vs parent **and** ha ≥ target; if **both** sides match, **both** stay — no single-winner prune. Shrink diffs **tag-lift** from the parent MCS when that already shows a cost drop (`atom_diff_after_cleavage`); else one MCS. Do **not** add a find_path cleave-only phase — treat cleaving edits like other reactions (`order_key` / filters). Redundancy across cleavage *sides* and free dealk orderings belongs on schema data (proposed ``cleave_side_group`` below), collected at expand — not a second predicate. Status: not decided for find_path Or wiring / side-group collection; graph + both-sides gate landed.
- **Yield: remapped free-step twins (tried).** `same_rule_maybe_skeleton` (rule multiset + Maybe side CSMIs + precedes under rule-name align) runs beside exact `same_linearizations` at hit yield. Not orbit-aware — unique-edit canonical sites make that unnecessary. Dominated-extension yield filter over-collapsed legitimate multipath (dimethylamine→PhCHO); dropped. Status: not decided for skeleton key.
...
- **cleave_side_group (cleavage analogue of ``swap_group``).** ResonancePair ends use ``swap_group`` so two *PatternInfo* ends of one pair are unordered iff they share a resolved group. Cleaving ``directed_bond`` / multi-fragment edits have two *sides* on one pattern (``site_map`` order: map 1 = oxygenated/leave carbon, map 2 = partner for Dealkylation). Proposal: optional per-side group labels on the cleaving pattern, resolved like ``swap_group`` (When override → PatternInfo → default). Defaults stay **unequal** (directed leave vs keep) so methyl dealkylation is not silently swappable; set both sides to the same group only for symmetric splits (e.g. azo). **Collection at expand:** group cleaving emissions that share the same resolved side-group signature + fragment-CSMI multiset into one Or / equivalence bag (both sides first-class; swappable sides normalize order in the key). Independent sites that share a leave-side group (several O-methyl dealks) are candidates for free-step plan equivalence without inventing precedes — exact mechanism not decided (yield key vs mid-search collect). Prefer data on ``PatternInfo`` / ``When``, not a find_path branch. Status: not decided — do not land the field until a caller + test name the sides (symmetric split and/or multipath dealk fold). Name bikeshed: ``cleave_side_group`` as ``tuple[str, str]`` aligned to ``site_map``, or two fields ``leave_group`` / ``keep_group``.
