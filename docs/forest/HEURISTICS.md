...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::SoftStack`.** After hard tiers `target_hit` then `novel_site` (vs already-yielded plans), soft lexicographic max-heap key:
  ...
  4. Among equal soft scores, [`pop_frontier`] alternates DFS (max `seq`) and BFS (min `seq`) each real expand/yield

  Child cost at enqueue for `cost_gain`; `lazy_closer` default false. **`delta_formula` is not a heap score** — soft mismatch warn/counter only. **Measured (HARD):** pure DFS+cost_gain eugenol 40M; **alternate DFS/BFS** eugenol **7680** (matches brief FIFO), tetraMeO 242k, veratrole 84k. Status: not decided for the soft stack vs simpler alternatives. Tests: `pop_frontier_alternates_dfs_and_bfs` / `heap_lack_of_improvement_counters_dfs` / `hop_cost_gain_is_parent_minus_child`.
