...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `MoleculeShells` / `AlignedShells` (n0/n1/n2).** Every heavy atom stores `aromatic` plus element bags `n0`/`n1`/`n2` that **include H** (ethane C: `n0=C:1`, `n1=C:1 H:3`, `n2=H:3`). Align → deltas + `unaligned_*`. **Site cost** = Σ over site atoms of normalized distance between `current + δ` and target (`site_shell_cost`). Each shell is `L1 / Σ max(|a|,|b|)` ∈ [0,1]; plus |Δaromatic| ∈ {0,1} (dearomatization hint). Per-atom ≤ 4 (≤ 3 when aligned n0 matches). Site atoms matched as a multiset (orbit/MCS safe). Cleaving sites expand with `site_atoms_with_leave`. δ = 0 is distance now; a complete edit scores 0. Close pairs use the **joint** site atom list. **Site bag** (`SiteShellBag`) for forecast↔applied checks; methyl leave is cleaved. **Baseline** for benches: raw `at_sites` Σ|δ| (pre-normalization). Status: not decided.
