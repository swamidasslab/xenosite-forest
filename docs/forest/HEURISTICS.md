...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::Match(MatchScoreSpec::log_neg_pc())`.** Heap key is score then `seq`. Score = `Σ −ln(1+child) − ln(1+parent)` over atom+formula (no `max(0,p−c)` flatten). Hard multipath p5: bill **1266** vs add-both/product 1309 / soft 1170. Former **add-both** = `ln` of Product factors (rank-identical to product-both) kept opt-in. Linear `Lin*`: hard p5 tied at 1538 (worse). Status: **approved** log-neg-pc as default (Rust derisk); SoftStack still best hard multipath bill — leave opt-in.
