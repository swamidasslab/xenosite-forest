"""Coverage for xenosite._archive_forest.base helpers and reaction-rule plumbing."""
...
from xenosite._archive_forest.base import (
    AromaticSystems,
    AtomTracker,
    ConjugatedSystems,
    EditMol,
...
def test_reactant_aligned_order_emits_hydrogens_and_stragglers():
    ...
    with patch("xenosite._archive_forest.base.Chem.GetMolFrags", return_value=[]):
...
def test_systems_kekulize_and_resonance_failures():
    ...
    with patch("xenosite._archive_forest.base.Kekulize", side_effect=ValueError):
        systems = ConjugatedSystems().systems(mol)
    assert systems

    with patch("xenosite._archive_forest.base.ResonanceMolSupplier", side_effect=ValueError):
...
def test_resfrags_resonance_supplier_fallbacks_and_none():
    ...
    with patch("xenosite._archive_forest.base.ResonanceMolSupplier", return_value=_Supplier()):
...
def test_base_module_main_runs_doctest():
    ...
    with patch("doctest.testmod") as mocked:
        runpy.run_module("xenosite._archive_forest.base", run_name="__main__")
