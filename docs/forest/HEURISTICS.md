...
- **Dehydrogenation heavy-neighbor match.** Pair ends whose effect is dearomatizing removes-H (not cleavage, not adds-O) require that each end atom's heavy neighbors on the **product** (after application) map onto exactly the target atom's heavy neighbors under some MCS view. Check runs post-keep in `find_path`, not in `filter_sites` / `pair_could_help` before the edit. Parent end atoms map onto the product via forest labels/tags. DH keeps connectivity; mismatched neighbors mean the applied edit did not produce what the target needs. Reads effect shape + ends / mapping / target — not a rule-name branch. Status: approved. Tests: `test_cycle_and_dh_neighbors.py`.
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Key: `(target_hit, novel_site, search_bias, site_progress, cost_gain, seq)` — soft scores + parent-relative **cost gain** first. Among equal scores, **`pop_frontier` alternates DFS (max seq) and BFS (min seq)** each real expand/yield (discard/dup does not flip). Child cost at enqueue for `cost_gain`; `lazy_closer` default false. **Measured (HARD):** pure DFS+cost_gain eugenol 40M; **alternate DFS/BFS** eugenol **7680** (matches brief FIFO), tetraMeO 242k, veratrole 84k. Status: not decided. Tests: `pop_frontier_alternates_dfs_and_bfs` / `heap_lack_of_improvement_counters_dfs` / `hop_cost_gain_is_parent_minus_child`.
...
- **``delta_formula: {element: delta}``** on `Effect` / possibility (zeros omitted). Declared net formula change, sealed from junction ``adds`` / ``removes`` bags **minus** ``leave_formula``. ForestMol caches formula as element→count (incl. H). Cleavage: named leave as negative counts, plus O/H at the cut from the bags. When OR arms disagree on the delta (halogen removal), each arm carries its own map under a ``When`` — do not invent a search branch. **Mismatch vs materialized product:** soft warn (`FormulaDeltaMismatchWarning` / `log::warn!`) and increment ``PathCounters.formula_delta_mismatch`` / EditCounters — never drops. Check runs in `ReactionRule.metabolize` **and** `find_path` (metabolites bypass metabolize). Suite ignores the warning under `filterwarnings` until annotations are complete; dedicated tests assert warn + count. Status: approved (Rust derisk + Python annotate). Redundancy with existing keys: see below.
