...
- **find_path ablation (`find_path_ablate`).** One-at-a-time vs baseline (eager, product-both, novel **off**, skeleton on, PhaseOne). Mid+hard, paths=1 and paths=5. No misses in any variant.
  ...
  | with-novel (was default) | 0 | 0 | 0 | **+997** |
  | no-skeleton | 0 | 0 | −117 | −821 |
  | no-epox-hyd | −24 | −26 | −244 | −264 |

  Read: single-path (p1) novel/skeleton are inert. SoftStack hurts hard p1, wins hard p5 (tetraMeO-naph). **Novel-site demotion** (former default) raised hard p5 bill almost 2× — left **off**. Skeleton-off lowers bill but fills `max_paths` with remapped twins — keep skeleton drop on. EpoxideHydration taxes every suite (~20–250 bill) without helping these quinone/dealk cases. Status: not decided for SoftStack default / EpoxideHydration PhaseOne tax; novel demotion **approved off**.

  **Propose drop / simplify (clarity, no miss risk on these benches):**
  1. **novel_site soft-demote** — **done**: default **off** (`deprioritize_known_site = false`).
  ...
  4. **skeleton twin yield-drop** — **keep** (on). Off looks “cheaper” only because twin hits pad `max_paths` and stop early; exact `same_linearizations` alone is not enough for remapped free steps. Watch `dropped_skeleton_twin` for how often search wastes a target hit on a remapped twin.
