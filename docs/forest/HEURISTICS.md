...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Trial — `MoleculeShells` / `AlignedShells` (n0/n1/n2).** Every heavy atom stores `aromatic` plus element bags `n0`/`n1`/`n2` that **include H**: center H in `n1`, H on dist-1 heavies in `n2` (ethane C: `n0=C:1`, `n1=C:1 H:3`, `n2=H:3`). Two molecules + alignment → same shape as **deltas**, plus `unaligned_reactant` / `unaligned_target`. Roles: (1) **site selection** — site-scoped shells (`AlignedShells::at_sites`); read **n0, n1, and n2 separately** (O+H in the **same** shell): alcohol `O:+1 H:−1`, carbonyl `O:+1 H:≤−2` — O sits in n1 at the attachment and n2 at the neighbor; do **not** sum bags across shells (that cancels alcohol H). Eval vs `candidate_could_help` on a small panel: recall 1.0, agree ~0.91, precision ~0.75 (FP = n2 bleed / over-broad cleave patterns). (2) **product closeness** — full-align `AlignedShells::cost` = Σ|δ|. Example: `eval_site_shells` / `show_matched_atoms`. Status: not decided.
