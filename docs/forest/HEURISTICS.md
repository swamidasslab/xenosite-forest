...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Key: `(target_hit, novel_site, search_bias, site_progress, seq)` — hits before non-hits, novel pattern+site before known, then soft scores (**`PatternInfo.search_bias`**, site H-progress), then **DFS** `seq` (LIFO among equal scores). Scores override DFS; not pure FIFO / not score-blind LIFO. Hydrogenation + OxygenReduction use `search_bias=-1`. **Measured (HARD, this shape):** eugenol-MeO `n_lin_sum` 1.8M / tetraMeO-naph 13M (matches prior DFS+demotions — site_progress mostly 0 on cleave/oxy edits, so DFS still dominates). Brief FIFO trial had better `n_lin` (7.7k / 80k) but was rejected as pure BFS. Status: not decided — need richer scores that override DFS on oxidative free-site sprawl. On pop, cheaply rescore; if the fresh key differs and is worse than peek, `heappush` back instead of expanding (compare-to-peek; no O(n) resort). Not a forest ranker. Tests: `heap_prefers_most_recently_queued_among_peers` / `heap_prefers_higher_search_bias_over_seq` / `heap_prefers_higher_site_progress_over_seq`.
