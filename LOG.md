...
- Focused tests: `test_forest_copy.py` + start_labels / rdkitutil / resonance / xf **32 passed**; broader rules/path suites green aside from pre-existing `test_path` butylbenzene/PhCH2OH budget asserts.

## 2026-09-20

- Audit vs earlier asks (post-`d30b6d5`): all seven were already in that commit — TautomerRule stub + docstring/TODO; `xenosite.net` / `find_network_paths` dropped (DROPPED approved, TODO line gone); thin `AtomTracker` xf facade + parity tests; forest-free `test_rdkit_behavior_ported`; nauty `all_site_pair_orbits_nauty` six families; DH `unique_orbit=bond_atom` + `swap_group` ordered/unordered; profile honesty note (below). Doc-only follow-up: HEURISTICS TautomerRule section; LOG no longer says nauty/DH “remain”; TODO tautomer line is SMARTS-matching only.
