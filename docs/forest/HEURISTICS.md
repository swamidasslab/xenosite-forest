...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current best / default — `HeapScoreMode::Match(MatchScoreSpec::close_improve_both())`.** After hard tiers `target_hit` then `novel_site`, match-family score from data `combine × metric` (not a SoftStack lex key). Recipe is **close×improve × both**: `closeness × improvement` with `formula_l1` (includes H) and atom_diff cost. Improvement = max(0, parent − child) + 1; closeness = `SCALE / (1 + child)`; both-metrics multiply factors. Plain [`BinaryHeap::pop`]. Bench: `find_path_bench --score close×improve-both` (aliases `match` / `product` / `product-both` / `close-x-improve-both`); `--matrix` runs all 9 + soft. Status: **approved** (Rust derisk) — current best after matrix + multipath (mid+hard); not switching to close-atom / improve-both.
