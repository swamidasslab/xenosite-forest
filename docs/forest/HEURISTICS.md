...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Key: `(target_hit, novel_site, search_bias, site_progress, cost_gain, seq)` — hits, novel pattern+site, soft scores (`search_bias`, site H-progress), then parent-relative **cost gain** (`parent_cost - child_cost`; ≤0 counters DFS), then **DFS** `seq`. When parent cost is known, child cost is computed at enqueue (tag-lift first, else MCS) so gain is not left unknown under `lazy_closer`; walk stores the diff for pop reuse. Soft only — non-closer still enqueued under lazy. **Measured (HARD):** tetraMeO-naph `n_lin_sum` 13M→242k; veratrole-allyl 371k→84k; eugenol-MeO **regressed** 1.8M→40M (larger local gains preferred into long detours). Status: not decided — parent-relative gain can still prefer locally cheap wrong branches. Tests: `heap_lack_of_improvement_counters_dfs` / `heap_prefers_larger_cost_gain_over_seq` / `hop_cost_gain_is_parent_minus_child`.
