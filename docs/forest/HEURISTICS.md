...
- **Lift child MCS via extend (MCS when not cost 0).** After an edit, tag-lift the parent MCS onto the product and **always extend** where the gap is placeable (unmapped product heavies onto free same-element target atoms bonded to mapped neighbor images — same rules as add-placement / `n_extra`). **Keep the lift only if `field_cost == 0`** (guaranteed — skip MCS). **Otherwise do not Aut-chase or trust a non-zero lift — use a fresh [`mcs_extend`](../../crates/xenosite-forest/src/atom_diff.rs)** (chematic MCS then the same placeable grow) and count `PathCounters::mcs_lift_rematch`. Tag-lift impossible → `mcs_lift_fallback` (expect zero). No double work of Aut-then-MCS. Correctness suite: `lift_mcs_correctness`. Status: approved (Rust derisk).
- **Fresh alignment is `mcs_extend`, not bare MCS.** Every `atom_diff` map is chematic MCS then placeable extend (prefer adjacency to every mapped neighbor image; else any one — so epoxide O can land on a hydroxyl). Same grow as lift. Do not call raw `find_mcs` for forest maps. Status: approved (Rust derisk). Tests: `epoxide_o_extends_onto_target_hydroxyl`.
...
- A richer walk-ranking key than target-hit + enqueue order. Status: not decided. Default is **`Match(close×improve-both)`** (formula × atom_diff close×improve); SoftStack and other `MatchScoreSpec` recipes remain opt-in (`--score`, `--matrix`). Plain max-heap pop. Sibling absolute-cost sort before enqueue preferred locally cheap dead ends on PhCH2OH→quinone — do not revive that. **Tried `PatternInfo.search_bias`:** SoftStack-only soft i8. **Dropped:** alternate DFS/BFS among equal scores. Matrix: close-atom lowest hard bill — switch not decided.
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape).
  ...
  **Current default — `HeapScoreMode::Match(MatchScoreSpec::close_improve_both())`.** After hard tiers `target_hit` then `novel_site`, match-family score from data `combine × metric` (not a SoftStack lex key). Default recipe is **close×improve × both**: `closeness × improvement` with `formula_l1` (includes H) and atom_diff cost. Improvement = max(0, parent − child) + 1; closeness = `SCALE / (1 + child)`; both-metrics multiply factors. Plain [`BinaryHeap::pop`]. Bench: `find_path_bench --score close×improve-both` (aliases `match` / `product` / `product-both` / `close-x-improve-both`); `--matrix` runs all 9 + soft. Status: approved (Rust derisk) as default pending matrix follow-up.
  ...
  **Match-score matrix (filter-only, plain pop; mid + hard).** Axes: combine = close | improve | close×improve; metric = atom | formula | both. SoftStack included as baseline. Cost = MCS HA gaps only; `formula_l1` includes H (no special `h_off`).
  ...
  | close×improve-both (default) | 79 | 260 | 0.99 |
  ...
  Read: mid bills collapsed (all 79; MeOPhOH edits up). Hard still ranks **close-atom** best; close×improve-both ahead of SoftStack. No misses. Vs prior HA+H cost (`|h_delta≠0|` in `field_cost`): hard close×improve-both was 217 / close-atom 210 — simplification costs ~40–50 hard bill. Whether to switch default to close-atom: not decided.
...
- **Epoxidation `dearomatizes` capability.** Catalog `Effect.dearomatizes=true` (aromatic epoxidation clears the ring bit at the site). [`PatternInfo::resolve_for_match`](../../crates/xenosite-forest/src/pattern.rs) resolves false on aliphatic site_map atoms (context mol, not Kekulé form). Site-shell residual reads the resolved bit for `|Δaromatic|`. Same resolve shape as Hydrogenation / pair `merge_dearomatizes`. Status: approved (Rust derisk). Tests: `epoxidation_dearomatizes_capability_resolves_on_aromatic_site` / `epoxidation_aromatic_residual_drops_with_dearomatic` / always-on [`pattern_info_catalog`](../../crates/xenosite-forest/tests/pattern_info_catalog.rs).
...
- **``search_bias: i8``** on `PatternInfo` (default `0`). Soft heap preference under **`HeapScoreMode::SoftStack`** after novel-site (not used by default `Match(close×improve-both)`). Higher pops sooner; negative demotes. **Hydrogenation** and **OxygenReduction** use `-1` (`demote_reductive`). Never drops or aborts. Pair emissions take `min(left, right)`. Status: not decided — schema for “less likely” still open; SoftStack-only score knob. Tests: `heap_prefers_higher_search_bias_over_seq` / `reductive_patterns_carry_negative_search_bias`.
