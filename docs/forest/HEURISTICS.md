...
- **Cleavage product graph → Or/Maybe, then non-cleavage across survivors.** Enumerate the cleavage layer (on demand / BFS); both fragment CSMIs are graph nodes. Fold arms by fragment multiset into Or. **MCS / `atom_diff` still gates** which fragments enqueue. Expand gate: strict cost drop vs parent **and** ha ≥ target; if **both** sides match, **both** stay — no single-winner prune. Shrink diffs **tag-lift** from the parent MCS when that already shows a cost drop (`atom_diff_after_cleavage`); else one MCS. **First-pass (tried):** `FindPathConfig.cleavage_first_depth` — inside `find_path`'s heap: walks with `cleavage_depth < d` expand cleaving only; once `cleavage_depth >= d`, **exclude cleave sites** (non-cleavage on survivors). find_path decides which paths to expand — not a separate seed handoff. Default **off**. Status: not decided.
