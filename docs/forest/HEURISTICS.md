...
- **Circular hydration ↔ dehydration.** True undo (hydrate / hydroxylate then dehydrate the *same* added oxygen, or the reverse) should not stay on a walk. A blanket “any O-add then any O-remove at overlapping atom index” is **not safe**: ethane → ethanol → ethene is Hydroxylation then Dehydration on the **same carbon** and is productive (beta-elim), not circular — test `hydroxylation_then_dehydration_same_site_yields_ethene` (ruleset = those two only). After several hops the same index can also be a different functional story. Need identity of the *added* oxygen (`WillAdd` / `AddedBy` / leave atom), not bare site overlap on `Effect.adds`/`removes`. Status: not decided — do not ship the broad site check.
...
- Lazy heap / stale-priority handling on the find_path frontier. Status: approved (shape). Minimal key: `(0 if target_hit else 1, seq)` — hits before non-hits, then **LIFO** `seq` (DFS bias: most recently queued pops first). Was FIFO; BFS spread on many unordered free sites explored redundant breadth. **Measured:** tetraMeO-naph bill 957→111 edits / 2.2s→0.27s, but eugenol-MeO / veratrole later hits got much noisier (`n_lin_sum` exploded; deep H₂/OR detours first). Status: not decided whether LIFO stays — trade bill vs plan quality. On pop, cheaply rescore; if the fresh key differs and is worse than peek, `heappush` back instead of expanding (compare-to-peek; no O(n) resort). Not a forest ranker. Test: `test_stale_priority_is_pushed_back` / `heap_prefers_most_recently_queued_among_peers`.
